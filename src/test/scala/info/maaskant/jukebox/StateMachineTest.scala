...
class StateMachineTest extends AnyFlatSpec with Matchers {
  ...
  "Uninitialized, any" should "Stopped, SignalReady" in {
    Uninitialized(Card.None) shouldBe(Stopped, Some(Initialize))
  }

  "Stopped, nothing" should "Stopped, None" in {
    val originalState = Stopped
    originalState(Card.None) shouldBe(originalState, None)
  }

  "Stopped, album" should "Playing, Play" in {
    Stopped(album1) shouldBe(playing(album1), play(album1))
  }

  "Stopped, unknown card" should "Stopped, None" in {
    val originalState = Stopped
    originalState(unknown) shouldBe(originalState, None)
  }

  "Stopped, shutdown" should "Stopped, Shutdown" in {
    Stopped(Card.Shutdown) shouldBe(Stopped, shutdown)
  }

  "Stopped, stop" should "Stopped, None" in {
    val originalState = Stopped
    originalState(Card.Stop) shouldBe(originalState, None)
  }

  "Playing, nothing" should "Paused, Pause" in {
    playing(album1)(Card.None, clock(instant1)) shouldBe(paused(album1, instant1), pause)
  }

  "Playing, same card" should "Playing, None" in {
    val originalState = playing(album1)
    originalState(album1) shouldBe(originalState, None)
  }

  "Playing, different card" should "Playing, Play" in {
    playing(album1)(album2) shouldBe(playing(album2), play(album2))
  }
  ...
  "Playing, shutdown" should "Playing, Shutdown" in {
    val originalState = playing(album1)
    originalState(Card.Shutdown) shouldBe(originalState, shutdown)
  }

  "Playing, stop" should "Stopped, Stop" in {
    playing(album1)(Card.Stop) shouldBe(Stopped, stop)
  }

  "Paused, nothing" should "Paused, None" in {
    val originalState = paused(album1, instant1)
    originalState(Card.None, clock(instant2)) shouldBe(originalState, None)
  }

  "Paused (HTTP URI), same card after short delay" should "Playing, Resume" in {
    paused(httpAlbum, instant1)(httpAlbum, clock(instant1.plusSeconds(59))) shouldBe(playing(httpAlbum), resume)
  }

  "Paused (HTTP URI), same card after long delay" should "Playing, Play" in {
    paused(httpAlbum, instant1)(httpAlbum, clock(instant1.plusSeconds(60))) shouldBe(
      playing(httpAlbum),
      play(httpAlbum)
    )
  }
  ...
  "Paused (other), same card after long delay" should "Playing, Resume" in {
    paused(album1, instant1)(album1, clock(instant1.plusSeconds(60))) shouldBe(playing(album1), resume)
  }

  "Paused, different card" should "Playing, Play" in {
    paused(album1)(album2) shouldBe(playing(album2), play(album2))
  }

  "Paused, unknown card" should "Paused, None" in {
    val originalState = paused(album1, instant1)
    originalState(unknown, clock(instant2)) shouldBe(originalState, None)
  }
  ...
  "Paused, stop" should "Stopped, Stop" in {
    paused(album1)(Card.Stop) shouldBe(Stopped, stop)
  }
